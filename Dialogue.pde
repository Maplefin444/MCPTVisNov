...
String getDialogue(JSONObject obj, int index) {
    return replaceName(obj.getJSONArray("data").getJSONObject(index).getString("dialogue"));
}
String getSpeaker(JSONObject obj, int index) {
    return replaceName(obj.getJSONArray("data").getJSONObject(index).getString("speaker"));
}
